function snew=SimLogit(B,s,eta)
%Simulates a generation update for an evolutionary game with Logit dynamics
% INPUT:
%	B:			payoff matrix
%	s:			Strategy state sequence matrix, has J rows (one per generation) and M columns (one per strategy)
%	eta:		Noise parameter for Logit dynamics; select by trial and error
% OUTPUT:
